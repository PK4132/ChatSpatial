...
def test_patch_numpy_testing_is_idempotent() -> None:
    ...
    try:
        ...
        noop_unpatch()
        # Wrapper is still active — legacy kwargs still work
        np.testing.assert_array_equal(x=np.array([1]), y=np.array([1]))
    finally:
        unpatch()
    # After real unpatch, no compat marker remains
    assert not getattr(
...
def test_patch_scipy_misc_derivative_adds_missing_attribute(
...
def test_patch_scipy_numpy_aliases_sets_missing_aliases(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scipy
...
def test_ensure_cellrank_compat_noop_when_not_numpy2(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
